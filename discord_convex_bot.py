...
def _load_command_guide() -> tuple[str, list[str]]:
    lines = [
        ...
        "Use /hunt or /thesis first. They are the clean hard-gated queues for the thesis: explorer-backed top10 whale control, Binance+Bitget trading evidence, low-float/FDV structure, 60D no-pump/dormancy, squeeze fuel, and early/no-chase tape.",
        "",
        "Primary queue:",
        ...
        "/thesis - plain-name alias for /hunt when you want the hard-gated core thesis queue without the RAVE/LAB microscope.",
...
def _add_base_thesis_context(frame: pd.DataFrame, *, min_whale_pct: float = 90.0) -> pd.DataFrame:
    rows = frame.loc[:, ~frame.columns.duplicated()].copy()
    if rows.empty:
        for column in ("_discord_holder_gate", "_discord_venue_gate", "_discord_no_pump_gate", "_discord_base_thesis_gate"):
            rows[column] = pd.Series(dtype=bool)
        return rows
    rows["_discord_holder_gate"] = _strict_top10_thesis_holder_gate_mask(
        rows,
        min_whale_pct=min_whale_pct,
        require_holder_evidence=True,
    )
    rows["_discord_venue_gate"] = _explicit_binance_bitget_trading_gate_mask(rows)
    rows["_discord_no_pump_gate"] = _no_recent_pump_proof_mask(rows)
    rows["_discord_base_thesis_gate"] = (
        _boolish_series(rows.get("_discord_holder_gate"), index=rows.index)
        & _boolish_series(rows.get("_discord_venue_gate"), index=rows.index)
        & _boolish_series(rows.get("_discord_no_pump_gate"), index=rows.index)
    ).fillna(False)
    return rows
...
def _base_thesis_status_text(row: pd.Series) -> str:
    if _boolish_scalar(row.get("_discord_base_thesis_gate")):
        return "baseThesis Y"
    blockers: list[str] = []
    if not _boolish_scalar(row.get("_discord_holder_gate")):
        blockers.append("holder")
    if not _boolish_scalar(row.get("_discord_venue_gate")):
...
def _load_thesis_list(
    ...
    *,
    ...
    _title, chunks = _load_crimepump_list(
        ...
        trigger=trigger,
        breakout_windows=breakout_windows,
        compact_title="Core thesis queue",
    )
    return "Core thesis queue", chunks
...
def main(*, force_disable_symbol_shortcuts: bool = False) -> None:
    ...
    thesis_kwargs = {"name": "thesis", "description": "Plain-name alias for the hard-gated core thesis queue."}
    if guild is not None:
        thesis_kwargs["guild"] = guild

    @tree.command(**thesis_kwargs)
    @app_commands.describe(
        min_tokens="Minimum token amount per confirmed transfer.",
        whale_flow_min_tokens="Minimum top-holder-origin CEX transfer amount for the whale-CEX trigger lane.",
        limit="Maximum rows to return.",
    ...
    async def thesis(
        ...
        if not _channel_allowed(interaction):
            await interaction.response.send_message("This command is locked to the configured alert channel.", ephemeral=True)
            return
        if not _tier_allows(_interaction_tier(interaction), _feature_required_tier("thesis")):
            await interaction.response.send_message(_access_denied_message("thesis"), ephemeral=True)
            return
        await interaction.response.defer(thinking=True)
        title, chunks = await asyncio.to_thread(
            _load_thesis_list,
