...
- strict 90%+ holder-concentration evidence gate with ETH/BNB/ARB chain, contract, and holder-source snapshot backing
...
- a `/precrime` radar for quiet latent setups after the hard holder-source snapshot, Binance+Bitget thesis gates, and default 60D no-pump/dormancy proof: holder/control concentration, target-CEX inventory tells, short-fuse perps, thin books, and no-chase low activity
- a primary `/radar` operator queue for the main thesis: top-10 whale-control threshold with ETH/BNB/ARB chain+contract holder-source snapshot evidence, Binance+Bitget, float/FDV trap evidence, 60D no-pump/dormancy, squeeze fuel, early/no-chase, and optional trigger filters for massive top-holder-origin whale-CEX flow, generic target-CEX flow, breakout highs, triggered-only, or core-watch rows
...
- symbol-level `/flowproof` and `/coincheck` views that separate verified transfer evidence from data gaps; `/flowproof` now prints compact thesis gates so transfer labels cannot masquerade as venue proof, and `/coincheck` enforces the Binance+Bitget thesis venue gate, 60D no-pump proof, and top-holder sender provenance when the transfer origin matches a scanned holder wallet
