...
- a dedicated `/ravelab` strict early-structure radar requiring observed 90%+ whale concentration with ETH/BNB/ARB chain+contract holder evidence, Binance+Bitget trading evidence, at least 60 days of history plus verified 60D closed-candle no-pump/no-chase dormancy, and a squeeze stack that pairs short crowding with perp/OI/liquidation/build fuel before ranking RAVE/LAB analogues by hard-gate completion first, with compact stage labels, blocker text, holder source/count/chain/contract evidence, 60D pump-proof source, venue provenance, optional top-holder-origin CEX-flow filtering, optional 1D/2D/3D/4D/etc high-breakout filtering after those hard gates, a blocked high-signal near-miss tail controlled by `near_miss_limit`, and `detail:true` for the full evidence stack
