...
def test_load_whale_dominance_list_defaults_to_top10_holder_concentration(monkeypatch) -> None:
    fresh = pd.DataFrame(
        [
            {
                "symbol": "LOWUSDT",
    ...
    assert "Matches: 1 | Base thesis gate: 0 | Core thesis gate: 0 | Showing: 1" in output
    assert "Diagnostic rows: /MEGAUSDT" in output
    assert "/MEGAUSDT | top10 91.0% | top100 98.0%" in output
    assert "baseThesis N blockers holder,BnBg,noPump60 | coreThesis N blockers base,float,shorts+fuel,notLate" in output
    assert "/WHALEUSDT" not in output
    assert "/LOWUSDT" not in output
    assert "diagnostic holder-concentration rows" in output


def test_load_whale_dominance_list_still_supports_top100_diagnostic_bucket(monkeypatch) -> None:
    fresh = pd.DataFrame(
        [
            {"symbol": "TOP100USDT", "top10_holder_pct": 70.0, "top100_holder_pct": 94.0, "scan_mode": "Deep", "scanned_at_utc": "now"},
            {"symbol": "TOP10USDT", "top10_holder_pct": 91.0, "top100_holder_pct": 98.0, "scan_mode": "Deep", "scanned_at_utc": "now"},
        ]
    )
    monkeypatch.setattr(bot, "_fresh_scanner_frame", lambda scan_mode=None, **kwargs: (fresh, "fresh Deep scan at now"))

    _, chunks = bot._load_whale_dominance_list(10, min_pct=90, bucket="top100")
    output = "\n".join(chunks)
...
def test_load_pump_watch_list_collapses_goal_stack_and_keeps_binance_targets(monkeypatch) -> None:
    fresh = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "recent_pump_60d_days": 60,
                "no_large_pump_60d_flag": True,
                "short_account_pct": 43.0,
                "short_dominance_score": 10.0,
                "short_account_build_score": 8.0,
                "short_liquidation_fuel_score": 5.0,
                "oi_delta_pct": 0.0,
                "dormant_short_fuse_score": 82.0,
                "pre_pump_precision_score": 76.0,
                "hour_return_pct": 1.6,
            ...
            },
            {
                ...
                "short_dominance_score": 80.0,
                "short_account_build_score": 74.0,
                "timing_too_late_score": 90.0,
                "hour_return_pct": 18.0,
                "day_return_pct": 82.0,
    ...
    assert "Holder evidence required: True" in output
    assert "Binance+Bitget required: True" in output
    assert "Gate rows: strict holder 4 | Binance+Bitget 4 | 60D no-pump 4 | Float/FDV 3 | Squeeze fuel 2 | Not-late 1" in output
    assert "Candidates: /PRIMEUSDT" in output
...
def test_load_ravelab_list_finds_early_historical_analogues(monkeypatch) -> None:
    ...
    assert "Core gates: top10 whale-control threshold with chain+contract explorer holder-source snapshot evidence" in output
    ...
    assert "Hard gates: top10 whale-control threshold with ETH/BNB/ARB chain+contract explorer holder-source snapshot evidence; Binance+Bitget; float/FDV trap; 60D no-pump/dormant; squeeze stack; early/no-chase." in crime_output
...
def test_strict_holder_evidence_requires_holder_source() -> None:
    ...
    goplus_only_text = bot._holder_evidence_text(frame.iloc[4])
    assert "needs explorer source" in goplus_only_text
