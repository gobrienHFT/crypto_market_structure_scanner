...
Dashboard-triggered Discord Convex alerts and the latest Convex cache re-score rows through the current core-thesis bucket logic before sending or writing, so stale `Convex Long` labels cannot bypass the low-float/FDV, short+squeeze-fuel, no-pump, holder, or Binance+Bitget gates.
