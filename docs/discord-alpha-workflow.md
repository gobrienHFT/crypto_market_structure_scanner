...
Use `/pumpwatch` as the fastest catch board after the same hard gates: observed top-10 holder concentration at or above 90%, ETH/BNB/ARB chain+contract holder-source snapshot evidence, Binance+Bitget trading evidence, and default 60D no-pump/dormancy proof. It does not force every row to have confirmed transfer evidence by default; it rank-orders target-CEX flow, whale/control, low float, short-squeeze fuel, timing, venue support, archetype match, and not-late risk into one watch state. Set `require_target_flow:true` when you only want verified Binance/Gate/Bitget transfer rows.
