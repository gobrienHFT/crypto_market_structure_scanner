...
Use `/alpha` as the triage queue. It first applies the strict thesis gate, observed 90%+ holder concentration with ETH/BNB/ARB chain+contract holder-source evidence plus Binance+Bitget trading evidence, then blends structure, timing, CEX-flow, scanner score, and short-account fuel into a compact watchlist.
